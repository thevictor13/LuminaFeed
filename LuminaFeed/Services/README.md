...
- `Subscriptions/` — per-user, per-feed notification channels (`ISubscriptionService`): the channel upsert
  (`SaveSubscriptionAsync`) + dialog state/prefill (`GetSubscriptionForEditAsync`) with `SaveSubscriptionRequest`
  validation (≥1 channel; a genuine Slack webhook when Slack is on), plus the email-only convenience and full
  unsubscribe. _(S3, C1)_
