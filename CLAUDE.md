...
- **UI** pages are thin (`@rendermode InteractiveServer`, no business logic), render `ErrorOr` errors through `Components/Shared/ErrorList`, carry prerendered data into the circuit with `[PersistentState]`, and read the user id from the **authenticated principal**, never from client input. Repeated buttons carry `aria-label`s naming their target.
