...
- [x] 🔎 **A-track review remediation.** A senior review of A1–A3 (no active bugs) added: an authenticated-non-admin denial E2E test (a forbidden user is redirected to `/Account/AccessDenied`); `FeedService.UpdateAsync` now clears `ETag`/`LastModified` on a URL change (forward-guard for C4); the shared `Modal` gained Escape-to-close + focus-on-open; extra dialog/service tests (edit-modal validation, modal cancel, second-admin delete, blank acting-id, Identity satellite cascade, zero-impact delete); and UI polish (clear success banner on modal open, live user-delete count, blank-acting-id refusal). Plan: [`A-review-remediation.md`](./A-review-remediation.md).

### Track B — Public UX
- [x] 🗂 **B1 — Feed list by category** (cards + image, 5/category, "more" +5, single-row desktop / wrap mobile). _(needs S2, A2)_ ✅ **Done** — a `CategorySection` component caps each category at 5 cards (grid `row-cols-…-lg-5`, single row on desktop / wrap on mobile) with a **More** button that reveals 5 more per click; the cap/"more" slice the already-loaded catalogue client-side (no re-query, no new `[PersistentState]`). Verified interactive in a real browser. Plan: [`B1-B2-public-list-paging-ordering.md`](./B1-B2-public-list-paging-ordering.md); feature: [`../features/public-feed-list.md`](../features/public-feed-list.md). _(A2 not required — the public list renders already-seeded feeds/images.)_
- [x] 🗂 **B2 — Ordering control** (popularity default, name; asc/desc) in category header. Popularity = the fixed `Feed.Popularity` figure seeded in G0.7. _(needs B1)_ ✅ **Done** — a per-category `OrderMenu` (Most popular / Least popular / Name A–Z / Name Z–A) re-sorts the already-loaded list in memory via `FeedOrdering.Sort` (default popularity-descending, matching the service). It's a **Blazor-driven** dropdown — the app loads no Bootstrap JS, so open/close (backdrop click-away + Escape) is handled in the circuit. Verified interactive in a real browser. Plan: [`B1-B2-public-list-paging-ordering.md`](./B1-B2-public-list-paging-ordering.md); feature: [`../features/public-feed-list.md`](../features/public-feed-list.md).
- [x] 🗂 **B3 — Category filter** (single category, cap 30, "more" +30). _(needs B1)_ ✅ **Done** — a top-of-page Blazor-driven `CategoryFilter` dropdown ("All feeds" + one item per category) narrows the home page to one category; `CategorySection.PageSize` becomes a parameter (5 on the main view, **30** when filtered, "more" adds that size), re-capped in memory over the already-loaded catalogue (no re-query, no new `[PersistentState]`). The selection is ephemeral (resets on refresh), matching B1/B2. Verified interactive in a real browser. Plan: [`B3-B4-category-filter-feed-page.md`](./B3-B4-category-filter-feed-page.md); feature: [`../features/public-feed-list.md`](../features/public-feed-list.md).
- [x] 🗂 **B4 — Feed detail page** (article cards: image, first paragraphs, title, subject to availability; subscribe/unsubscribe button). _(needs G0.3; content from S4)_ ✅ **Done** — `/feed/{id:guid}` ([`Feed.razor`](../../LuminaFeed/Components/Pages/Feed.razor)) shows the feed header, a top **Visit site** + **Subscribe** bar (reusing the home page's subscribe pattern; only a `bool? Subscribed` is persisted, articles re-queried), and the newest 50 articles as `ArticleCard`s; unknown ids show a not-found message. Each public card gained an internal **View articles** button (kept alongside *Visit site*). `IFeedService.GetFeedDetailAsync` orders articles **in memory** (newest first, dateless last) — **EF's SQLite provider throws on a `DateTimeOffset` in `ORDER BY`**, so the P1.R "order by `PublishedAt` in SQL" note does not hold; the client-side sort matches the notification path, and no article index/migration was needed. Verified interactive in a real browser. Plan: [`B3-B4-category-filter-feed-page.md`](./B3-B4-category-filter-feed-page.md); feature: [`../features/feed-page.md`](../features/feed-page.md).

### Track C — Subscriptions & notifications
- [x] 🗂 **C1 — Full subscribe flow** (dialog with email + Slack switches; unsubscribe-in-red state; per-channel unsubscribe). **Enforce the `SlackEnabled ⇒ SlackWebhookUrl` invariant** (and the `https://hooks.slack.com/services` prefix rule) here via FluentValidation — the `Subscription` entity documents but does not enforce it. _Already done by S3 + P1.R:_ unauthenticated → login/register preserving the return location (`ReturnUrl` through login, registration, the confirmation email and the confirm page's Continue button), the red Unsubscribe state, accessible button names. _(needs S3, auth)_ ✅ **Done** — the `SubscribeDialog` (a Blazor-driven Bootstrap modal, no JS interop) with email + Slack switches, a validated/`LastOrDefault`-prefilled webhook, per-channel unsubscribe and an in-dialog full unsubscribe; `SaveSubscriptionAsync` (validated channel upsert) + `GetSubscriptionForEditAsync` on `ISubscriptionService`. Slack **delivery** stays C2 (a Slack-only sub receives nothing yet). Verified in a real browser (installed-Chrome PuppeteerSharp probe): the circuit is interactive and the full flow — subscribe, Slack reveal, webhook rejection, valid save, per-channel unsubscribe, full unsubscribe — runs with **0 console/page errors**. Plan: [`C1-full-subscribe-flow.md`](./C1-full-subscribe-flow.md); feature: [`../features/subscriptions.md`](../features/subscriptions.md). Review remediation (webhook-validation hardening, shared-`Modal` a11y; C1.R.3/C1.R.4 deferred): [`C1.R-review-remediation.md`](./C1.R-review-remediation.md).
...
**Cross-track dependencies:** `A3` needs subscriptions (`C1`) · `C5` needs `C3` · `C4` needs `C2`+`C3`. _(`B1` was originally listed as needing `A2`, but it does not — the public list renders already-seeded feeds/images, so B1–B2 intentionally shipped ahead of A2; see the B1 done-note above.)_
