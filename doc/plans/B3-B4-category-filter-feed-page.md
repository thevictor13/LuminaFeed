...
- New `Task<FeedDetail?> GetFeedDetailAsync(Guid feedId, int maxArticles, CancellationToken)`: project the feed to `FeedSummary` (return `null` when absent), then the newest `maxArticles` of its articles ordered `OrderByDescending(a => a.PublishedAt).ThenByDescending(a => a.FetchedAt)`. SQLite sorts NULL `PublishedAt` last under `DESC` (dateless articles fall to the bottom); correct text ordering of the stored `DateTimeOffset` relies on the **UTC invariant**.

### Persistence — `(FeedId, PublishedAt)` index (migration)
Add `builder.HasIndex(a => new { a.FeedId, a.PublishedAt });` to `Data/Configurations/ArticleConfiguration.cs` (mirrors the `(CategoryId, Popularity)` index added in G0.8 for the default sort), then generate an EF migration so the model and migrations stay in sync — the `HasPendingModelChanges` drift guard (G0.9) fails otherwise, and startup auto-applies it.
- First verify EF tooling: `DOTNET_CLI_TELEMETRY_OPTOUT=1 dotnet ef --version` (else `dotnet tool restore` / `dotnet tool install dotnet-ef` into the local tool manifest — NuGet restore is permitted). Then `dotnet ef migrations add AddArticleFeedPublishedIndex` (writes only under the working dir; does not touch any DB). Fallback if tooling is unavailable: hand-author the migration + snapshot edit mirroring `Data/Migrations/20260921210325_AddDomainModel.cs`.
